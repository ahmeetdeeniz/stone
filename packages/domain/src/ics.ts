...
export function exportCalendarIcs(items: readonly CalendarItem[]): string {
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Stone//Calendar 1.0//EN", "CALSCALE:GREGORIAN"];
  for (const source of items) {
    ...
    if (item.allDay) {
      ...
      lines.push(`DTSTART:${compactInstant(item.startAt!)}`, `DTEND:${compactInstant(item.endAt!)}`);
    }
...
export function importCalendarIcs(
  ...
  if (new TextEncoder().encode(source).length > ICS_MAX_BYTES) throw new Error("Calendar file is too large.");
...
function parseDateValue(value: string, timezone: string, allDay: boolean): { date: string; instant: string | null } {
  ...
  if (/^\d{8}T\d{6}Z$/u.test(value)) {
    ...
    return { date: new Intl.DateTimeFormat("en-CA", { timeZone: timezone }).format(new Date(instant)), instant };
  }
  if (/^\d{8}T\d{6}$/u.test(value)) {
    const date = expandDate(value.slice(0, 8));
    return { date, instant: zonedWallTimeToInstant(date, `${value.slice(9, 11)}:${value.slice(11, 13)}`, timezone, "earlier") };
  }
  throw new Error("Calendar date value is unsupported.");
}

function serializeRule(value: CalendarRecurrence): string {
  const frequency = value.frequency === "weekdays" ? "WEEKLY" : value.frequency === "custom" ? value.unit.toUpperCase() + "LY" : value.frequency.toUpperCase();
...
function parseRule(value: string | undefined): CalendarRecurrence | null {
  ...
  const fields = Object.fromEntries(value.split(";").map((part) => part.split("=", 2))) as Record<string, string>;
  ...
  if (!["daily", "weekdays", "weekly", "monthly"].includes(frequency ?? "") || !Number.isInteger(interval) || interval < 1) return null;
  return { frequency: frequency as CalendarRecurrence["frequency"], interval, unit: frequency === "monthly" ? "month" : frequency === "weekly" || weekdays ? "week" : "day", preferredDayOfMonth: null, untilDate: fields.UNTIL ? expandDate(fields.UNTIL.slice(0, 8)) : null };
}
...
function find(map: Map<string, string>, name: string): string | undefined { return property(map, name)?.value; }
function compactDate(value: string): string { return value.replaceAll("-", ""); }
function compactInstant(value: string): string { return value.replaceAll("-", "").replaceAll(":", "").replace(/\.\d{3}Z$/u, "Z"); }
...
function addDays(value: string, count: number): string {
  const date = new Date(`${value}T00:00:00Z`); date.setUTCDate(date.getUTCDate() + count); return date.toISOString().slice(0, 10);
}
function escapeText(value: string): string { return value.replaceAll("\\", "\\\\").replaceAll("\n", "\\n").replaceAll(",", "\\,").replaceAll(";", "\\;"); }
function unescapeText(value: string): string { return value.replace(/\\n/gu, "\n").replace(/\\([\\,;])/gu, "$1"); }
function nullableText(value: string | undefined): string | null { const text = value ? unescapeText(value).trim() : ""; return text || null; }
function parseCategory(value: string | undefined): CalendarItem["category"] { return ["neutral", "purple", "blue", "green", "amber", "red"].includes(value ?? "") ? value as CalendarItem["category"] : "purple"; }
function foldLine(value: string): string { const chunks = value.match(/.{1,73}/gu) ?? [""]; return chunks.join("\r\n "); }
function stableHash(value: string): string { let hash = 0x811c9dc5; for (const character of value) hash = Math.imul(hash ^ character.charCodeAt(0), 0x01000193); return (hash >>> 0).toString(36); }
